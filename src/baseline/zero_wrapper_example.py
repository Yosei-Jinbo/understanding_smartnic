...
class ZeroWrapperExample(Module):
    ...
    def forward(self, *inputs, **kwargs):
        ...
        # all forward kernels finish, so nsys attributes them to this range.
    ...
    def backward(self,
        ...
        # Profile-only: keep :ZeroWrapperExample.backward NVTX range open until
        # all backward kernels finish, so nsys attributes them to this range.
        if os.environ.get("NSYS_SYNC_RANGES", "0") == "1":
            torch.cuda.synchronize()
