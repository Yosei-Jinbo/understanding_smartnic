...
def printflock(*msgs):
    """Print messages from all concurrent gpus without getting interleaved text
    (useful when debugging multi-gpu sync issues)."""
    global fcntl
    if fcntl == None:
        import fcntl
...
def log_rank_file(rank, *msgs):
    """Print to a per-rank log file (log_rank_<rank>.txt); useful for debugging
    hangs in sync processes by diffing the per-rank logs."""
    global fh
    if fh is None:
