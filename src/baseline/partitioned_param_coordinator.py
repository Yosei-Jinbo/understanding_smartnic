...
class PartitionedParameterCoordinator:
    ...
    @instrument_w_nvtx
    def __all_gather_params(self, params: Set[Parameter]) -> None: #分割済パラメータ群に対して非同期AllGatherを起動し、inflightレジストリにハンドルを記録
        """for each partitioned parameter, kick off an async allgather and store
        the work handle for the in flight parameters."""
        partitioned_params = []
        for param in params:
            if param.ds_status == ZeroParamStatus.NOT_AVAILABLE:
                partitioned_params.append(param) #まだ手元にない（NOT_AVAILABLE）ものだけAllGather対象に追加
                self.__n_available_params += param.ds_numel

        if partitioned_params:
            with torch.cuda.stream(self.__allgather_stream):
                handle = partitioned_params[0].all_gather_coalesced(partitioned_params) #複数paramsをまとめてall_gather

            for param in partitioned_params:
                assert param.ds_status == ZeroParamStatus.INFLIGHT, param.ds_summary()
                self.__inflight_param_registry[param] = handle #各パラメータの状態がINFLIGHTになったことを確認し、共通ハンドルを登録（同一coalesced呼び出しを共有）
