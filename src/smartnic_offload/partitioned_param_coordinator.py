...
class PartitionedParameterCoordinator:
    ...
    def fetch_sub_module(self, current_submodule: Module) -> None:
        ...
        if dist.get_rank() == 0:
            ps = list(iter_params(current_submodule))  # recurse=False のまま（今の実装に合わせる）
            ds_total = sum(p.ds_numel for p in ps)
            max_p = max([p.ds_numel for p in ps], default=0)

            print(
                f"STEP {self.__step_id} M{current_submodule.id}({type(current_submodule).__name__}) "
                f"n_params={len(ps)} ds_total={ds_total} max_param_ds_numel={max_p}"
            )
        '''
