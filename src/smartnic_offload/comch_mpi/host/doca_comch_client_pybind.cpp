                                          ...
                                          uint64_t root = static_cast<uint64_t>(-1))
{
    ...
    int rc = 0;
    {
        ...
        rc = ucp_collective_enqueue_u64(id, src_addr, src_size, dst_addr, dst_size, req, &handle);
    }
    if (rc < 0) {
        throw std::runtime_error("ucp_collective_enqueue_u64 failed: rc=" + std::to_string(rc));
    }
    return handle;
}

/* GPU flag completion sync 付き enqueue。
 * flag_gpu_addr: pinned host memory 上の int32 slot アドレス (gpu_flag_pool.py が採番)
 * flag_value:    完了時に DPU が RDMA Write する generation 値 */
                                          ...
                                          uint64_t root = static_cast<uint64_t>(-1))
{
    ...
    {
        py::gil_scoped_release release;
        rc = ucp_collective_enqueue_with_flag_u64(id, src_addr, src_size, dst_addr, dst_size,
                                                  req, flag_gpu_addr, flag_value, &handle);
    }
    if (rc < 0) {
        throw std::runtime_error("ucp_collective_enqueue_with_flag_u64 failed: rc=" + std::to_string(rc));
    }
    return handle;
}

/* GPU flag pool (pinned host int32 配列) の DPU 登録 (一回限り) */
static void register_flag_pool_py(uint64_t base_addr, uint64_t total_len)
{
    doca_error_t ret;
    {
        py::gil_scoped_release release;
        ret = comch_register_flag_pool(base_addr, total_len);
    }
    if (ret != DOCA_SUCCESS) {
        throw std::runtime_error("comch_register_flag_pool failed");
    }
}
...
static void comch_req_wait_py(uint64_t handle)
{
    ...
    if (rc < 0) {
        throw std::runtime_error("comch_req_wait_u64 failed: rc=" + std::to_string(rc));
    }
}
...
static void doca_comch_client_init_py(py::str server_name, py::str pci_addr)
{
    ...
    comch_client_init(g_server_s.c_str(), g_pci_s.c_str());
}

PYBIND11_MODULE(doca_comch_client_pybind, m) {
    /* enum 値の番号は DPU との wire contract — 本体 (comch_mpi_common.h) は変更しない。
     * Python 側で使う ALL_GATHER / REDUCE_SCATTER のみ export する。 */
