#!/bin/bash
# DeBERTa-XL prefetch + CPU トレース (osrt/sampling/ctxsw). 表B 用 (純 GPU 計算時間計測用)
...
export NSYS_OUTPUT_BASE="logs/nsys/deberta_xl_buf_prefetch_cputrace"
...
torchrun \
  ...
  2>&1 | tee "logs/nsys/deberta_xl_buf_prefetch_cputrace_$(hostname)_node${NODE_RANK}.log"
