#!/bin/bash
# ViT-L/16 prefetch + CPU トレース (osrt/sampling/ctxsw). 表B 用 (純 GPU 計算時間計測用)
...
export NSYS_OUTPUT_BASE="logs/nsys/vit_l_16_buf_prefetch_cputrace"
...
torchrun \
  ...
  2>&1 | tee "logs/nsys/vit_l_16_buf_prefetch_cputrace_$(hostname)_node${NODE_RANK}.log"
